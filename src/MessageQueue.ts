...
const INITIALISE_WORKER = 'INITIALISE_WORKER';
...
export const createWorker = async (worker: Worker, config: any = {}): Promise<MessageQueue> => {
  ...
  await new Promise<void>((resolve) => {
    messageQueue.once(INITIALISE_WORKER, () => {
      messageQueue.once(WORKER_INITIALISED, () => {
        resolve()
      })
      messageQueue.sendEvent(WORKER_READY, {})
...
export const receiveWorker = async (callback: (messageQueue: MessageQueue) => void): Promise<void> => {
  const messageQueue = new MessageQueue(globalThis as any)
  await new Promise<void>((resolve) => {
    const interval = setInterval(() => {
      messageQueue.sendEvent(INITIALISE_WORKER, {})
      messageQueue.sendQueue()
    }, 100)
    messageQueue.once(WORKER_READY, () => {
      clearInterval(interval)
      callback(messageQueue)
      messageQueue.sendEvent(WORKER_INITIALISED, {})
      messageQueue.sendQueue()
      resolve()
    })
    messageQueue.sendQueue()
